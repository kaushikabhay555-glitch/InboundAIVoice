...
class _LiveKitNoiseFilter(logging.Filter):
    """
    Suppress known-harmless LiveKit SDK log messages that clutter the console:

    1. "unexpected message type: 258"
       aiohttp PING frame (type 258). aiohttp auto-sends PONG; this is just
       the SDK not explicitly whitelisting PING before logging.

    2. "failed to connect to livekit, retrying in 0s"
       The server closes idle WebSocket connections after ~30 min; the SDK
       reconnects instantly (0 s back-off) and re-registers successfully.
       It is cosmetically a warning but functionally a no-op.
    """
    _SUPPRESSED = (
        "unexpected message type: 258",
...
_lk_filter = _LiveKitNoiseFilter()
logging.getLogger("livekit.agents").addFilter(_lk_filter)
...
from livekit.plugins import openai, sarvam, groq as groq_plugin
...
def get_ist_time_context() -> str:
    ...
    return (
        ...
        f"Use the following table for dates:\n{days_block}\n"
        f"Internal instruction: Use ISO format for tool calls. Do NOT speak dates in ISO format to the user; use natural language like 'Tuesday at 10 AM'.]"
    )


# ── Language presets ──────────────────────────────────────────────────────────
LANGUAGE_PRESETS = {
    "hinglish":    {"label": "Hinglish (Hindi+English)", "tts_language": "en-IN", "tts_voice": "kavya",  "instruction": "Speak in natural Hinglish — mix Hindi and English like educated Indians do. Default to Hindi but use English words when more natural."},
    "hindi":       {"label": "Hindi",                   "tts_language": "hi-IN", "tts_voice": "ritu",   "instruction": "Speak only in pure Hindi. Every reply MUST contain at least one Devanagari Hindi word. Avoid pure English sentences."},
...
# ── External imports ──────────────────────────────────────────────────────────
import db
from calendar_tools import get_available_slots, async_create_booking
from notify import (
...
class AgentTools(llm.ToolContext):
    ...
    @llm.function_tool(description=(
        "Save and CONFIRM a booking. Call this once you have the caller's name, phone, preferred date, and time. "
        "Pass start_time as a FULL ISO 8601 string like '2026-05-09T10:00:00+05:30'. "
        "If you only have a time (e.g. '10:00 AM'), also pass appointment_date as 'YYYY-MM-DD'."
    ))
    async def save_booking_intent(
        self,
        start_time:       Annotated[str, "Full ISO 8601 datetime '2026-05-09T10:00:00+05:30' OR time only '10:00 AM'"],
        caller_name:      Annotated[str, "Full name of the caller"],
        caller_phone:     Annotated[str, "Phone number of the caller"],
        appointment_date: Annotated[str, "Date in YYYY-MM-DD format (required if start_time is time-only)"] = "",
        notes:            Annotated[str, "Any notes, email, or special requests"] = "",
    ) -> str:
        logger.info(f"[TOOL] save_booking_intent: {caller_name} at {start_time} (date_hint={appointment_date})")
        try:
            # Resolve start_time to a full ISO 8601 datetime (handles time-only strings)
            from calendar_tools import _parse_iso_start_time
            iso_start = _parse_iso_start_time(start_time, date_hint=appointment_date)
            ...
            if result.get("success"):
                ...
                self.caller_name = caller_name
                # Use natural language in return string so LLM speaks it naturally
                dt = datetime.fromisoformat(start_time.replace("+05:30", ""))
                readable_time = dt.strftime("%A, %B %d at %I:%M %p")
                return f"SUCCESS: The appointment is CONFIRMED for {caller_name} on {readable_time}. Inform the user it is confirmed."
            else:
                return f"FAILED: {result.get('message')}. Tell the user you had an issue and will call them back."
        except Exception as e:
            logger.error(f"[TOOL] save_booking_intent failed: {e}")
            return "ERROR: Internal booking failure. Tell the user you'll call them back shortly."
    ...
    async def check_availability(
        ...
        try:
            ...
            # Cal.com returns {"time": "2026-03-01T10:00:00+05:30", "label": "10:00 AM"}
            slot_strings = [s.get("label") or s.get("time", "")[11:16] for s in slots[:6]]
            slot_strings = [s for s in slot_strings if s]  # drop blanks
...
class OutboundAssistant(Agent):
    ...
    # ── Patterns that must NEVER reach TTS ───────────────────────────────
    # These are tool-call artifacts that the small LLM leaks into text output.
    _CODE_RE = re.compile(
        r'</?[a-zA-Z_][a-zA-Z0-9_]*/?>|'   # <function_calls>, </function_calls>, <invoke>
        r'\{\s*"[a-zA-Z_]+"\s*:|'           # {"name":  or {"arguments":
        r'```[\s\S]*?```|'                   # ```code block```
        r'`[^`]{1,120}`|'                    # `inline code`
        r'\bsave_booking_intent\b|'          # tool name spoken
        r'\bcheck_availability\b|'
        r'\bget_business_hours\b|'
        r'\btransfer_call\b|'
        r'\bend_call\b|'
        r'\bfunction_call\b|'
        r'\btool_call\b|'
    ...
    def __init__(self, agent_tools: AgentTools, first_line: str = "", live_config: dict | None = None):
        ...
        # Anti-code / anti-leak rules (#40)
        safety_rules = (
            "\n\n[CRITICAL SPEECH RULES — NEVER VIOLATE]\n"
            "1. You are speaking on a phone call. Output ONLY natural spoken words.\n"
            "2. NEVER output JSON, XML, function names, angle brackets, curly braces, or any code syntax — not even a single character.\n"
            "3. NEVER say phrases like 'save booking intent', 'function call', 'tool call', 'ISO', 'UTC', 'ID', 'SUCCESS:', 'FAILED:', or similar technical terms.\n"
            "4. NEVER output raw markdown, asterisks, bullet points, hyphens, or numbered lists.\n"
            "5. When a tool call succeeds, translate the result into ONE warm conversational sentence. Say nothing else.\n"
            "6. If you encounter a technical error, simply say: 'Main ek second mein wapas aata hoon' or 'Give me just a moment' and move on.\n"
            "7. Keep every reply to 1-2 short spoken sentences maximum. No more."
        )
        
        final_instructions = base_instructions + ist_context + lang_instruction + safety_rules
    ...
    # ── Real-time LLM output sanitizer ─────────────────────────────────────
    # Intercepts every token from the LLM before it reaches TTS.
    # Strips code artifacts that the model leaks when it confuses text output
    # with function-call output (common in small models like llama-3.1-8b).
    async def llm_node(
        self,
        ...
        async for chunk in super().llm_node(chat_ctx, tools, model_settings):
            if isinstance(chunk, llm.ChatChunk) and chunk.delta and chunk.delta.content:
                original = chunk.delta.content
                cleaned  = self._CODE_RE.sub("", original)
                # Only modify the chunk if regex ACTUALLY removed something.
                # Do NOT .strip() — streaming tokens have meaningful leading spaces
                # (e.g. ' slots', ' ko', ' ek') that separate words. Stripping them
                # causes words to run together and fires false-positive log entries.
                if cleaned != original:
                    # Collapse any double-spaces left behind by mid-word removal
                    cleaned = re.sub(r'  +', ' ', cleaned)
                    logger.debug(f"[LLM-FILTER] Code artifact stripped from LLM output")
                    chunk.delta.content = cleaned if cleaned.strip() else None
            yield chunk
...
async def entrypoint(ctx: JobContext):
    ...
    tts_language  = live_config.get("tts_language", "en-IN")  # en-IN accepts English+Hindi; hi-IN rejects pure English
    ...
    for key in ["LIVEKIT_URL","LIVEKIT_API_KEY","LIVEKIT_API_SECRET","OPENAI_API_KEY","GROQ_API_KEY",
    ...
    if llm_provider == "groq":
        agent_llm = groq_plugin.LLM(
            model=llm_model or "llama-3.1-8b-instant",
            max_completion_tokens=300,  # minimum for tool-call JSON + spoken reply
        )
    ...
    # (Sentence chunker removed — livekit-agents 1.5.7 no longer supports before_tts_cb.
    # Response length is controlled by the "max 2 sentences" rule in agent_instructions.)
    ...
    call_start_time = datetime.now(pytz.utc)  # timezone-aware for astimezone() to work correctly

    # ── Recording → Supabase Storage ─────────────────────────────────────
    egress_id = None
    _s3_key    = os.environ.get("SUPABASE_S3_ACCESS_KEY", "")
    _s3_secret = os.environ.get("SUPABASE_S3_SECRET_KEY", "")
    _s3_ep     = os.environ.get("SUPABASE_S3_ENDPOINT", "")
    if _s3_key and _s3_secret and _s3_ep:
        try:
        ...
        logger.debug("[RECORDING] S3 keys not set — skipping recording.")
    ...
    # ── Participant connection logging ────────────────────────────────────
    # NOTE: session.start() already invokes agent.on_enter() automatically.
    # We only log participant events here — do NOT call on_enter() manually
    # or the greeting will fire multiple times.
    @ctx.room.on("participant_connected")
    def on_participant_connected(participant: rtc.RemoteParticipant):
        logger.info(f"[CONNECT] Participant connected: {participant.identity}")

    for p in ctx.room.remote_participants.values():
        logger.info(f"[CONNECT] Participant already in room: {p.identity}")
    ...
    def on_participant_disconnected(participant):
        ...
        agent_is_speaking = False
        # NOTE: do NOT call unified_shutdown_hook here — it's already registered
        # via ctx.add_shutdown_callback() below. Calling it here would cause it
        # to run twice (double Telegram messages + duplicate DB records).
    ...
    async def unified_shutdown_hook(shutdown_ctx: JobContext):
        ...
        if agent_tools.booking_intent:
            ...
            # If already confirmed during call, just send notifications
            if agent_tools.booking_confirmed and intent.get("booking_id"):
                notify_booking_confirmed(
                ...
                booking_status_msg = f"Booking Previously Confirmed: {intent.get('booking_id')}"
            
            # Fallback: if intent was saved but not yet booked
            elif intent.get("start_time"):
                result = await async_create_booking(
                ...
                if result.get("success"):
                    ...
                    booking_status_msg = f"Booking Confirmed at Shutdown: {result.get('booking_id')}"
                else:
                    booking_status_msg = f"Booking Failed: {result.get('message')}"
            else:
                booking_status_msg = "Booking Skipped: no slot time captured"
        else:
            notify_call_no_booking(
        ...
        # Sentiment analysis (#14) — uses whichever LLM provider is active
        sentiment = "unknown"
        if transcript_text and transcript_text != "unavailable":
            try:
                ...
                _sentiment_prompt = [
                    {"role": "user", "content":
                     f"Classify this call as exactly one word: positive, neutral, negative, or frustrated.\n\n{transcript_text[:800]}"}
                ]
                if llm_provider == "groq":
                    ...
                    _sentiment_model = "llama-3.1-8b-instant"  # production model, 6k TPM
                else:
        ...
        # Save to Supabase — wrapped so a missing table never crashes shutdown
        try:
            db.save_call_log(
                phone=caller_phone,
                duration=duration,
            ...
            logger.warning(f"[SHUTDOWN] DB save skipped (table missing or connection error): {e}")
